/* =================================================================================
 * vanity_kernel.cl
 * OpenCL kernel for high-speed Tox vanity address generation.
 * WARNING ! Unstable !
 * ================================================================================= */
...
// constants
__constant uint32_t ge25519_ec2d[10] = {0x0a2a561, 0x02379b3, 0x0216cc2, 0x0345d1d, 0x021136c, 0x038aa88, 0x02ebe34, 0x034a742, 0x02c0c63, 0x00873d6};
__constant uint32_t ge25519_ecd[10] = {0x14d3a5b, 0x00a7469, 0x02c4823, 0x0375a03, 0x02313de, 0x00c7333, 0x039e557, 0x018b320, 0x0356973, 0x004d1b1};
__constant uint32_t ge25519_sqrtneg1[10] = {0x004924d, 0x022397c, 0x03a2c58, 0x0182496, 0x03b9074, 0x0203a01, 0x019421b, 0x030398a, 0x002e071, 0x014594c};
...
void fill_random_bytes(uchar *buffer, uint len, uint *rng_state) {
    for (uint i = 0; i < len; i += 4) {
        uint random_val = xorshift_rng(rng_state);
        buffer[i] = (uchar)(random_val);
        if (i + 1 < len) buffer[i + 1] = (uchar)(random_val >> 8);
        if (i + 2 < len) buffer[i + 2] = (uchar)(random_val >> 16);
        if (i + 3 < len) buffer[i + 3] = (uchar)(random_val >> 24);
    }
}

ushort calculate_checksum(const uchar *data) {
    ushort checksum = 0;
    for (int i = 0; i < 36; ++i) { // 32 байта pubkey + 4 байта nospam
        checksum ^= data[i];
...
void bin_to_hex(char *hex, const uchar *bin, const int len) {
    __constant char *hex_chars = "0123456789ABCDEF";
    for (int i = 0; i < len; ++i) {
...
__kernel void find_vanity_address(
    ...
    uint rng_state = gid + (uint)start_nonce;
    ...
    while (*found_flag == 0) {
        ...
        ge25519_scalarmult_base_niels(&r, s);
        
        ge25519_pack(public_key, &r);

        for(int i=0; i<32; ++i) tox_address_data[i] = public_key[i];
        U32TO8_LE(tox_address_data + 32, nospam); 

        ushort checksum = calculate_checksum(tox_address_data);
        tox_address_data[36] = (uchar)(checksum);
        tox_address_data[37] = (uchar)(checksum >> 8);
        
        bin_to_hex(tox_address_hex, tox_address_data, wanted_len);

        int match = 1;
        for (uint i = 0; i < wanted_len; ++i) {
            if (tox_address_hex[i] != wanted_prefix[i]) {
                match = 0;
                break;
            }
        }
